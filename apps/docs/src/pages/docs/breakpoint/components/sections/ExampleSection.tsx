...
const ExampleSection = () => (
  <section>
    ...
    <Document.Paragraph mb={6}>
      ...
      <code className="font-code rounded-md bg-neutral-100 px-1.5 py-0.5 text-blue-600">breakpoints</code> prop에 객체를 전달하여 커스텀
      값을 사용하세요.
    </Document.Paragraph>
    ...
    <Document.Paragraph mb={6}>
      <code className="font-code rounded-md bg-neutral-100 px-1.5 py-0.5 text-blue-600">targetRef</code>를 사용하면 viewport가 아닌 특정
    ...
    <div className="w-full space-y-3">
      <Breakpoint breakpoints={customBreakpoints} down="mobile">
        <div className="rounded-lg bg-neutral-100 p-4">
          <p className="text-sm font-medium">📱 Mobile (0px ~ 599px)</p>
        </div>
      </Breakpoint>

      <Breakpoint breakpoints={customBreakpoints} only="tablet">
        <div className="rounded-lg bg-neutral-100 p-4">
          <p className="text-sm font-medium">💻 Tablet (600px ~ 1199px)</p>
        </div>
      </Breakpoint>

      <Breakpoint breakpoints={customBreakpoints} up="desktop">
        <div className="rounded-lg bg-neutral-100 p-4">
          <p className="text-sm font-medium">🖥️ Desktop (≥1200px)</p>
        </div>
      </Breakpoint>
    </div>
  );
        ...
        <div className="space-y-2">
          <Breakpoint targetRef={containerRef} breakpoints={customBreakpoints} up="md">
            <div className="rounded-lg bg-neutral-100 p-3">
              <p className="text-sm font-medium">컨테이너 너비 ≥400px (md 이상)</p>
            </div>
          </Breakpoint>

          <Breakpoint targetRef={containerRef} breakpoints={customBreakpoints} down="md">
            <div className="rounded-lg bg-neutral-100 p-3">
              <p className="text-sm font-medium">컨테이너 너비 {"<"}400px (md 미만)</p>
            </div>
          </Breakpoint>
        </div>
      </div>
...
>
  <p className="text-sm text-neutral-600 mb-3">우측 하단을 드래그하여 크기를 조절하세요</p>
  
  <div className="space-y-2">
    <Breakpoint targetRef={containerRef} up="md">
      <div className="rounded-lg bg-neutral-100 p-3">
        <p className="text-sm font-medium">컨테이너 너비 ≥768px</p>
      </div>
    </Breakpoint>

    <Breakpoint targetRef={containerRef} down="sm">
      <div className="rounded-lg bg-neutral-100 p-3">
        <p className="text-sm font-medium">컨테이너 너비 ≤640px</p>
      </div>
    </Breakpoint>
  </div>
</div>`;
