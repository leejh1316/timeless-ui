import { Button, Calendar, Tabs, Toast, Tooltip } from "@timeless-ui/ui";
import {
  ArrowRight,
...
export default function Dashboard() {
  ...
  const handleStartWriting = () => {
    setShowToast(true);
    // navigate("/learning-log"); // Toast 확인을 위해 잠시 주석 처리 또는 Toast 후 이동 로직 추가 가능
  };
    ...
    <Toast.Provider>
      <div className="min-h-screen bg-gray-50 text-gray-900 antialiased">
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="before:bg-linear-to-b relative flex items-center justify-between overflow-hidden rounded-3xl bg-white p-10 shadow-sm transition-transform before:absolute before:left-0 before:top-0 before:h-full before:w-1 before:from-teal-600 before:to-transparent hover:-translate-y-0.5 hover:shadow-md">
                <div>
                  ...
                  <Tooltip.Root>
                    <Tooltip.Trigger className="cursor-help bg-transparent p-0 hover:bg-transparent">
              ...
              <div>
                ...
                <div className="rounded-3xl bg-white px-8 py-6 shadow-sm">
                  <Tabs.Root defaultValue="all">
                    <Tabs.List className="mb-6 flex gap-6 border-b border-gray-100">
                      <Tabs.Trigger
                        value="all"
                        className="cursor-pointer pb-3 text-sm font-semibold text-gray-400 transition-colors data-[state=active]:border-b-2 data-[state=active]:border-teal-600 data-[state=active]:text-teal-600"
                      >
                        전체
                      </Tabs.Trigger>
                      <Tabs.Trigger
                    ...
                    {["all", "pending", "completed"].map((tabValue) => (
                      <Tabs.Content key={tabValue} value={tabValue} className="flex flex-col gap-1">
                        {ACTIVITY_DATA.filter(
                          (item) =>
                            tabValue === "all" ||
                          ...
                          <div
                            key={index}
                            className="flex cursor-pointer items-center rounded-xl p-3 transition-colors hover:bg-gray-50"
                          >
        ...
        >
          ...
          <div className="flex-1">
            <Toast.Title className="mb-1 text-sm font-bold text-gray-900">작성 시작</Toast.Title>
            <Toast.Description className="text-xs text-gray-500">
              12주차 학습일지 작성을 시작합니다. 임시저장은 자동으로 수행됩니다.
            </Toast.Description>
          </div>
