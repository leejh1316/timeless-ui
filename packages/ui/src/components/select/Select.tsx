...
const SelectRoot = ({
  ...
  return <SelectProvider scope={__scopeSelect} {...contextValue}>{children}</SelectProvider>;
};
...
const SelectValue = ({ placeholder = "Select an item", __scopeSelect }: { placeholder?: string, __scopeSelect?: Scope }) => {
  ...
  return <Primitive.span>{text}</Primitive.span>;
};
...
const SelectIcon = ({ children }: { children?: React.ReactNode }) => {
  return <Primitive.span aria-hidden="true">{children || "▼"}</Primitive.span>;
};
...
const SelectContent = forwardRef<React.ComponentRef<typeof Primitive.div>, ScopedProps<PrimitivePropsWithRef<'div'>>>(
  ({ children, className, style, __scopeSelect, ...props }, ref) => {
    const { floating, isOpen, listRef, labelsRef } = useSelectContext(CONTENT_NAME, __scopeSelect);
    const { context: floatingContext, refs, floatingStyles, getFloatingProps, placement } = floating;
    const composedRef = useComposedRefs(refs.setFloating, ref);
...
const SelectItem = forwardRef<React.ComponentRef<typeof Primitive.div>, ScopedProps<SelectItemProps>>(
  ({ children, className, value, disabled, textValue: textValueProp, __scopeSelect, ...props }, ref) => {
    const {
      ...
      registerItem,
      multiple,
    } = useSelectContext(ITEM_NAME, __scopeSelect);
...
export const Arrow = forwardRef<SVGSVGElement, React.SVGProps<SVGSVGElement> & { __scopeSelect?: Scope }>(
  (props, ref) => {
    ...
    return (
        <FloatingArrow ref={ref} context={floating.context} width={Number(width)} height={Number(height)} {...(rest as any)} />
    );
  }
);
