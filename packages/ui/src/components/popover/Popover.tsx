...
const PopoverRoot = ({ children, __scopePopover,...options }: ScopedProps<PopoverProps>) => {
  ...
  return <PopoverContextProvider scope={__scopePopover} {...popover}>{children}</PopoverContextProvider>;
};
...
const PopoverTrigger = forwardRef<React.ComponentRef<typeof Primitive.div>, ScopedProps<PopoverTriggerProps>>((props,forwardedRef) => {
  ...
  return (
    <Button ref={composedRef} data-state={isMounted ? "open" : "closed"} {...otherProps} {...getReferenceProps()}/>
  );
});
...
const PopoverView = forwardRef<React.ComponentRef<typeof Primitive.div>,ScopedProps<PrimitivePropsWithRef<'div'>>>((props,forwardedRef)=>{
  const {__scopePopover,...otherProps} =props
  ...
  return (
    <Primitive.div ref={composedRef} style={floatingStyles} data-state={isMounted ? "open" : "closed"} {...otherProps} {...getFloatingProps()}/>
  );
})
...
const PopoverContent = forwardRef<React.ComponentRef<typeof Primitive.div>,ScopedProps<PrimitivePropsWithRef<'div'>>>((props,forwardedRef)=>{
  const {__scopePopover,...otherProps} =props
  const { transitionStatus, isMounted, transitionStyle } = usePopoverContext(POPOVER_CONTENT_NAME, __scopePopover);
  return (
    <Primitive.div ref={forwardedRef} style={{...transitionStyle}} data-status={transitionStatus} data-state={isMounted ? "open" : "closed"} {...otherProps}/>
  );
});
